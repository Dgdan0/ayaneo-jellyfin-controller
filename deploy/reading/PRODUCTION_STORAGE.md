...
| Acquisition workspace | `E:\Downloads\Reading` | BookKeeprr and qBittorrent read/write at `/downloads` and the compatibility alias `/media/downloads`; invisible to readers |
...
`/downloads` after completion. BookKeeprr 1.1.1 sends qBittorrent the legacy
container path `/media/downloads/incomplete`, so the same dedicated host
workspace is also mounted at `/media/downloads` in both acquisition
containers. This is an alias of `E:\Downloads\Reading`, not access to the
managed library. The qBittorrent `temp_path_enabled` preference must be
enabled; setting only the temp path is not sufficient. BookKeeprr validates
and imports from that workspace into `/media`; qBittorrent never receives a
mount for either library root.
...
9. Confirm qBittorrent's default path is `/downloads/`, its incomplete path is
   `/downloads/incomplete/`, `temp_path_enabled` is true, the shared download
   root is visible at `/media/downloads` in both acquisition containers, and
   BookKeeprr connects to Docker host `qbittorrent:18080`.
